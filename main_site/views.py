...
def main(request):
	...
	number = fill_number()
	list = OfferElement.objects.filter(to_fix=False).order_by('-id')[:number]
	return render(request, 'index.html', {'list':list, 'list_fix':list_fix, 'category':category})
...
def category(request, slug): #Представление товаров
	...
	return render(request, 'forsome.html', {'cards':cards, 'category':category, 'page':page, 'filter':filter, 'subcat':all_subcategory})
