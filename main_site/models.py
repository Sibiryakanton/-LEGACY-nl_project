...
class Category(models.Model):
    ...
    photo = models.ImageField("Фото", upload_to="images/catalog", null=True)
...
class Subcategory(models.Model):
    ...
    title = models.CharField("Заголовок", max_length=30)
    slug = models.CharField("Метка (для URl-адреса)", max_length=30, null=True)
...
class CategoryElement(models.Model):
    ...
    subcategory = models.ForeignKey('Subcategory', verbose_name='Подкатегории', default=0, blank=True, on_delete=models.CASCADE)
...
class Sponsor(models.Model):  # Карточка спонсора для раздела "Бизнес-проект"
    class Meta:
        verbose_name = "Лидер"
    ...
    photo = models.ImageField("Фото", upload_to="images/sponsor", null=True)
    refferal_url = models.CharField('Реферальная ссылка', max_length=200, null=True)
    ...
    def __str__(self):
        title = "{0} {1} {2} ({3})".format(self.name, self.last_name, self.middle, str(self.code_number))
        return title
